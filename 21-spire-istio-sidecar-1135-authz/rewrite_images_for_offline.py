...
# Same tool as 17-/19-/20-'s rewrite_images_for_offline.py, copied here
# (not imported) so this directory stays self-contained — re-hosts every
# `image: <original-registry>/<path>:<tag>` line under <registry-host>,
# keeping path+tag, writing output into manifests-offline/ alongside
# manifests/ (left untouched for online use).

DEFAULT_FILES = [
    "manifests/spire-1152-cluster1.yaml",
    "manifests/spire-1152-cluster2.yaml",
    "manifests/spiffe-csi-driver.yaml",
    "manifests/istio-client.yaml",
    "manifests/istio-server.yaml",
]
